...
description: IWR6843 원시 ADC 데이터가 이더넷으로 도착하지 않고 보드 발열이 동반된 상황에서 전원·제어·LVDS·FPGA·Ethernet 경로를 분리해 점검한 트러블슈팅 사례.
...
IWR6843의 원시 ADC 데이터를 PC로 받기 위해 DCA1000EVM을 연결했지만 Ethernet
수집이 시작되지 않았고 보드 발열도 함께 나타났다. 이때 코드부터 바꾸면 설정 문제와
하드웨어 문제를 섞어 볼 가능성이 컸다. 먼저 실제 장비 상태를 남기고, 데이터가 지나가는
경로와 장치를 시작시키는 경로를 따로 따라갔다.
...
## 화면과 LED를 관측점으로 삼았다 {#observations}
...
<figure><img src="{{ '/assets/images/projects/mmwave-visualizer/mmwave-studio-readregister-error.webp' | relative_url }}" alt="FTDI Connected, RS232와 SPI Disconnected, ReadRegister error -11을 표시한 mmWave Studio 화면" width="1200" height="509"><figcaption>FTDI 인식 이후에도 RS232·SPI·device detection이 다음 단계로 넘어가지 못한 시점.</figcaption></figure>
...
- PC 측 Ethernet 설정과 capture 상태를 하드웨어 확인 결과와 별도로 비교했다.
...
납땜 보수와 반복 설정 뒤에도 Ethernet 원시 ADC streaming은 복구되지 않았다. FPGA
내부 buffer가 정상으로 샘플을 받았는지, 특정 부품이 고장 났는지는 확정하지 못했다.
...
이 경험에서 얻은 핵심은 실패 현상에 곧바로 원인 이름을 붙이지 않는 방법이다. 전원·발열,
설정·상태 전이, connector·solder, LVDS·buffer와 Ethernet을 단계별 관측점으로 만들었다.
