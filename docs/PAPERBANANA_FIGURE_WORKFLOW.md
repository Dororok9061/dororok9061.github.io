...
5. Compare labels, arrows, stages, component names, and every number with the source.
6. Refine only the candidate that remains source-faithful.
7. Export the final PNG to the exact `output` path from the manifest.
8. Add a WebP derivative and update the related page/gallery metadata.
